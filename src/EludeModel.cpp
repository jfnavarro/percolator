...
string
RTModel::feature_groups[NO_FEATURE_GROUPS] = { "krokhin_index",
    "krokhin100_index",
    "krokhinc2_index",
    "krokhintfa_index",
    "doolittle_index",
...
RTModel::RTModel() :
  ...
  for (int i = 0; i < NO_FEATURE_GROUPS; i++)
    if (selected_features & 1 << i) noFeaturesToCalc
      += no_features_per_group[i];
}
    ...
    *(features++) = cst;
    ...
    for (int i = 4; i <= (peptide.length() - 5); ++i) {
      ...
      if (i == 4) {
        ...
        if (hWindow < min) min = hWindow;
        if (hWindow > max) max = hWindow;
      }
...
double* RTModel::hydrophobicMoment(const float *index,
  ...
  if (lengthPeptide > w) {
    for (int i = 1; i <= lengthPeptide - w; ++i) {
      ...
      if (hMoment > maxHMoment) maxHMoment = hMoment;
      if (hMoment < minHMoment) minHMoment = hMoment;
    }
  }
...
double RTModel::noHydrophobicAA(const string& peptide) {
  ...
  for (unsigned int ix = 0; ix < peptide.size(); ++ix)
    if (hydrophobicAA.find(peptide[ix]) != string::npos) noOccurences++;
...
double RTModel::noConsecHydrophobic(const string& peptide) {
  ...
  for (unsigned int ix = 0; ix < (peptide.size() - 1); ++ix) {
    ...
    if ((isH1 != string::npos) && (isH2 != string::npos)) noOccurences++;
  }
...
void RTModel::trainRetention(vector<PSMDescription>& trainset,
  ...
  if (svm_check_parameter(&data, &param) != NULL) {
    if (VERB >= 1) cerr
          << "ERROR: Incorrect parameters for the SVR. Execution aborted. "
          << endl;
    exit(-1);
  }
...
double RTModel::computeKfoldCV(const vector<PSMDescription> & psms,
  ...
  if (VERB > 2) cerr << k << " fold cross validation..." << endl;
...
double RTModel::computeSimpleEvaluation(
  ...
  if ((VERB >= 2) && (noPsms < test_frac * 10u)) cerr
        << "Warning: very little data to calibrate parameters (just "
        << noPsms << "), parameter values may be unreliable" << endl;

  if (VERB > 2) cerr << "Simple evaluation..." << endl;
...
void RTModel::trainSVM(vector<PSMDescription> & psms) {
  ...
  if (VERB >= 2) cerr << "Building the model..." << endl;
  ...
  for (it1 = grids.gridGamma.begin(); it1 != grids.gridGamma.end(); ++it1) {
    for (it2 = grids.gridC.begin(); it2 != grids.gridC.end(); ++it2) {
      for (it3 = grids.gridEpsilon.begin(); it3 != grids.gridEpsilon.end(); ++it3) {
        ...
        if (VERB > 2) cerr << "Error = " << error << endl;
  ...
  if (saveCalibration) {
    calFile.close();
    if (VERB > 3) cerr << "Calibration details were saved to "
...
void RTModel::copyIndexModel(svm_model* from) {
  // destroy an existant model
  if (index_model != NULL) svm_destroy_model(index_model);
...
double RTModel::computeKfoldCVIndex(const vector<PSMDescription> & psms,
  ...
  if (VERB > 2) cerr << k << " fold cross validation on " << noPsms
                       << " psms..." << endl;
...
void RTModel::trainIndexSVRNoCCalibration(vector<PSMDescription> & psms,
  ...
  if (VERB >= 2) cerr << "Building the hydrophobicity model with c = ..."
                        << C << endl;
...
void RTModel::trainIndexSVR(vector<PSMDescription> & psms) {
  ...
  if (VERB >= 2) cerr << "Building the hydrophobicity model..." << endl;
...
void RTModel::setGridType(const GridType & g) {
  ...
  if (g == NO_GRID) {
    ...
    if (VERB >= 2) cerr << g << "is unknown. Execution aborted." << endl;
    exit(-1);
  }
}
...
void RTModel::saveSVRModel(const string modelFile,
  ...
  svm_save_model2(modelFile.c_str(),
                  model,
                  PSMDescription::normSub,
                  PSMDescription::normDiv,
                  theNormalizer->getSub(),
                  theNormalizer->getDiv(),
                  theNormalizer->getNumRetFeatures(),
                  selected_features);
}
...
void RTModel::loadSVRModel(const string modelFile,
  ...
  model = svm_load_model2(modelFile.c_str(),
                          &PSMDescription::normSub,
                          &PSMDescription::normDiv,
                          theNormalizer->getSub(),
                          theNormalizer->getDiv(),
...
void RTModel::printFeaturesInUse(ostringstream & oss) {
  ...
  for (int i = 0; i < NO_FEATURE_GROUPS; i++)
    if (selected_features & 1 << i) oss << feature_groups[i] << "\t";
  oss << endl;
}
