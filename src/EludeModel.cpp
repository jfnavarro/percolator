...
string RTModel::feature_groups[NO_FEATURE_GROUPS] =
		{ "inhouse_index", "krokhin_index", "krokhin100_index", "krokhinc2_index", "krokhintfa_index", "doolittle_index",
		  "hessa_index", "peptide_size", "no_ptms", "ptms", "bulkiness", "polar_features", "hydrophobic_features", "aa_features", };
// number of features in each group
int RTModel::no_features_per_group[NO_FEATURE_GROUPS] = {17, 17, 17, 17, 17, 17, 17, 1, 1,  ptmAlphabet.size(), 1, 2, 2, aaAlphabet.size()};
...
static int DEFAULT_FEATURE_GROUPS = 1 + 32 + 128 + 1024 + 2048 + 4096 + 8192;
...
void RTModel::fillFeaturesAllIndex(const string& pep, double *features)
{
	...
	if(doKlammer)
	{
		...
		features = fillAAFeatures(aaAlphabet,peptide, features);
		 ...
		 features = fillAAFeatures(aaAlphabet, peptide.substr(peptide.size()-2,1), features);
...
double* RTModel::fillAAFeatures(const string & alphabet, const string& pep, double *feat)
{
...
void RTModel::trainIndexSVR(vector<PSMDescription> & psms)
{
  ...
  grid_e.assign(NORMAL_GRID_EPSILON, NORMAL_GRID_EPSILON + (sizeof(NORMAL_GRID_EPSILON) / sizeof(NORMAL_GRID_EPSILON[0])));
...
void RTModel::saveSVRModel(const string modelFile, Normalizer *theNormalizer)
{
	...
	if (selected_features & 1 << 0)
	{
		...
		for (int i = 0; i < inhouseIndexAlphabet.length(); ++i)
			fp << " " << inhouseIndexAlphabet[i] << " " << our_index[inhouseIndexAlphabet[i] - 'A'];
		fp << endl;
	}
