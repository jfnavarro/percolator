...
// path to the library
string RTPredictor::libPath = "/scratch/lumi_work/projects/retention_time/results/article_results/lib/";
...
RTPredictor::RTPredictor() : trainFile(""), testFile(""),outputFile(""),
			 ...
			 decayingPeptidesFile(""), hydrophobicityFile(""), removeRedundant(false), autoModelSelection(false),
			 linearAdjust(true), addLibModel(false), removeDecaying(false), testIncludesRT(true), removeNEnzymatic(false),
			 theEnzyme(NULL)
{
...
bool RTPredictor::parseOptions(int argc, char **argv)
{
	...
	cmd.defineOption("y", "no_in_source", "Specifies that in source fragments should be removed from the test set",
			"", TRUE_IF_SET);
	...
	cmd.defineOption("g", "hydrophobicity", "Specifies a file where the hydrophobicity index trained will be saved", "filename");
	cmd.defineOption("b", "lts_coverage", "Specifies the fraction of data used in calibrating a model via LTS", "value");
...
void RTPredictor::printRunInformation()
{
	...
	oss << "----------------------------------------" << endl;
	oss <<  "Running Elude v" << VERSION << " with the following options: " << endl;
	if (!trainFile.empty())
	...
	if (!hydrophobicityFile.empty())
		oss << " *Saving hydrophobicity model to file: " << hydrophobicityFile << endl;
	...
	if (removeDecaying)
		oss << " *In source fragments will be removed from the test set" << endl;

	if (!decayingPeptidesFile.empty())
		oss << " *In source fragments are saved to file " << decayingPeptidesFile << endl;
									...
									size_t noFeatures)
{
	// number of rt related features
 	...
 	if (VERB > 3)
 		cerr << endl << "Initializing features table for " << noRTFeatures << " features and " << numberRecords << " records..." << endl;
...
void RTPredictor::removeRedundantPeptides(vector<PSMDescription> & psms, const char* dataset)
{
	if (VERB > 2)
		cerr << endl << "Removing redundant peptides from " << dataset << "... " << endl;
	int initial, final;
...
void RTPredictor::removeSourceCIDs(vector< pair< pair<PSMDescription, bool>, bool> > & psms)
{
	...
	{
		if (removeDecaying)
			cerr << endl << "Removing in source fragments from train and test sets..." << endl;
		else
			cerr << endl << "Removing in source fragments from train set..." << endl;
	}
...
void RTPredictor::writeDecayToFile(vector< pair <pair<PSMDescription, bool>, bool> > & psms)
{
	...
	outDecay << "# File generated by Elude; all decaying peptides are listed below " << endl;
...
void RTPredictor::trainSVRRegressor()
{
	...
	if (!hydrophobicityFile.empty())
	{
		if (model.calculateIndex())
			...
			cerr << endl << "The option too generate a hydrophobicity index is not on, therefore no index is generated." << endl;
	}
...
void RTPredictor::loadSVRModel()
{
	...
	if (trainFile.empty() || model.isModelNull())
	{
		...
		{
			if (VERB > 2)
				cerr << endl << "Error: model file " << loadModelFile << " is inaccessible. Please check the file location and permissions and then try again. ";
			exit(1);
		}
...
void RTPredictor::loadBestModel()
{
	...
	if (trainPsms.empty())
	{
		if (VERB > 2)
		{
			cerr << "Warning: No training data available to select model. " << endl;
...
void RTPredictor::writeOutputFile(vector<PSMDescription> & psms)
{
	...
	{
		if (testIncludesRT)
			out << (it->peptide) << "\t" << it->getPredictedRetentionTime() << "\t" << it->getRetentionTime() << "\n";
		else
...
void RTPredictor::run()
{
	...
	if(!testFile.empty())
	{
		...
		else
		{
			...
			for(int i = 0; i < testPsms.size(); ++i)
				cerr << testPsms[i].peptide << " " << testPsms[i].predictedTime << endl;
		}
	}
...
double RTPredictor::computeWindow(vector<PSMDescription> & psms)
{
	...
	if (VERB > 2)
		cerr << endl << "Computing " << fractionPeptides*100 << "% window..." << endl;
	...
	while (i <= k)
	{
		diff = (psms[i + nr - 1].getPredictedRetentionTime() - psms[i + nr - 1].getRetentionTime()) -
			   (psms[i].getPredictedRetentionTime() - psms[i].getRetentionTime());
		if (diff < win)
			win = diff;
		i++;
	}
