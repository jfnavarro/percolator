...
void LTSRegression::runLTS()
{
	...
	if (VERB > 3)
	{
		...
		cerr << "   h = " << h << " = " << percentageH*100 << "%, no_initial_subsets = " << noSubsets << ", epsilon = " << epsilon << endl;
	}
	...
	if (VERB > 2)
		cerr << "Final LTS equation: y = " << regCoefficients.first << " * x + " << regCoefficients.second << endl;
